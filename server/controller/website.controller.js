...
export const generateWebsite=async(req,res,next)=>{
    try {
        ...
        if(user.credits < 50){
          return next(handleError(400,"you have not enough credits to generate a website"))
        }
        ...
        if(!parsed.code){
          ...
          return next(handleError(400,"ai returned invalid response"))
        }
        ...
        return next(handleError(500,`generate website error ${error}`))
    }
}
...
export const getWebsiteById=async(req,res,next)=>{
  try {
    ...
    if(!website){
      return next(handleError(400,"website not found"))
    }
    
    return res.status(200).json(website)
  } catch (error) {
    return next(handleError(500,`get website by id error ${error}`)) 
  }
}

export const changes=async(req,res,next)=>{
  try {
    const { prompt } = req.body
    if(!prompt){
      return next(handleError(400,"prompt is required"))
    ...
    return next(handleError(500,`update website error ${error}`))
  }
}
...
export const getAll =async(req,res,next)=>{
  try {
    ...
    return next(handleError(500,`get all website error ${error}`))
  }
}
...
export const deploy=async(req,res,next)=>{
  try {
    ...
    return next(handleError(500,`deploy website error ${error}`))
  }
}
...
export const getBySlug=async(req,res,next)=>{
  try {
    ...
    return next(handleError(500,`get by slug error ${error}`))
  }
}