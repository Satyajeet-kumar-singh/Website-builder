...
function Pricing() {
    ...
    const handleBuy=async(planKey)=>{
      ...
      setLoading(planKey)
      try {
        const result = await axios.post(`${import.meta.env.VITE_API_BASE_URL}/billing`,{planType:planKey},{withCredentials:true})
    ...
    <div className='relative min-h-screen overflow-hidden bg-[#050505] text-white px-6 pt-16 pb-24'>
     <div className='absolute inset-0 pointer-events-none'>
       ...
       <div className='absolute bottom-0 right-0 w-[400px] h-[400px] bg-purple-600/20 rounded-full blur-[120px]'/>
        ...
        onClick={()=>{navigate("/")}}><ArrowLeft size={16}/> Back</button>
        <motion.div
        ...
        <div className='relative z-10 max-w-7xl mx-auto grid grid-cols-1 md:grid-cols-3 gap-8'>
          {plans.map((p,i)=>(
            ...
            className={`relative rounded-3xl p-8 border backdrop-blur-xl transition-all ${p.popular ? "border-indigo-500 bg-linear-to-b from-indigo-500/20 to-transparent shadow-2xl shadow-indigo-500/20" : "border-white/10 bg-white/5 hover:border-indigo-400 hover:bg-white/10"}`}>
              {p.popular && (
              ...
              <div className='flex items-end gap-1 mb-4'>
                  ...
                  {p.name !== "Free" && <span className='text-sm text-zinc-400 mb-1'>/one-time</span>}
              </div>

              <div className='flex items-center gap-2 mb-8'>
                <Coins size={16} className='text-yello-400'/>
